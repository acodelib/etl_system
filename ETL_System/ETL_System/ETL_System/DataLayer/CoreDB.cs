...
namespace ETL_System {
    public class CoreDB {
        ...
        public DataTable adminGetUsers() {
            DataTable t = etl_database.Tables["User"].Copy();

            return t;
        }

        public void adminAddUser(User u, string pass) {
            DataRow r;
            //add new user
            lock (_locker) {
                r = etl_database.Tables["User"].NewRow();
                r["role_id"] = u.role == "admin" ? 1 : 2;
                r["login"] = u.login;
                r["password"] = pass;
                r["is_active"] = true;
                r["last_conn_timestamp"] = DateTime.Now;

                using (SqlDataAdapter db_adapter = new SqlDataAdapter("Select * from ETL_System.dbo.[User]", SystemSharedData.app_db_connstring)) {
                    var builder = new SqlCommandBuilder(db_adapter);
                    etl_database.Tables["User"].Rows.Add(r);
                    DataRow[] target = { r };
                    db_adapter.Update(target);
                }

                etl_database.Tables["User"].Reset();
                using (SqlDataAdapter db_adapter = new SqlDataAdapter("Select * from ETL_System.dbo.[User]", this.connection_string)) {
                    db_adapter.Fill(this.etl_database, "User");
                }
            }
        }

        public void adminDeleteUser(List<int> u) {
            List<DataRow> deleted_rows = new List<DataRow>();
            //1.remove from DataTable element by element
            using (SqlDataAdapter db_adapter = new SqlDataAdapter("Select * from ETL_System.dbo.[User]", SystemSharedData.app_db_connstring)) {
                var builder = new SqlCommandBuilder(db_adapter);                
                    lock (_locker) {
                        foreach (int us in u) {
                            DataRow r = etl_database.Tables["User"].Select($"user_id = '{us}'")[0];
                            r.Delete();
                            deleted_rows.Add(r);
                        }
                        //2.propagate to DB
